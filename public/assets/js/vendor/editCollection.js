(function ($) {
    ...
    var colJs = {
      ...
      userIssuedCollections: function () {
        $.ajax({
            type: "GET",
            url: "/aaaab7bdafd53fc4193c0bbbe1e91f0c",
            success: function(result) {
              if (!(Array.isArray(result))) {
                ...
                result.forEach(element => {
                  html += `<div class="chip">
                    <img src="${element.collectionLogo}"/>
                    <span class="family">${element.displayName}</span>
                    <span class="name">${element.family}</span>
                    <span style="display: none;" class="taxon">${element.taxon}</span>
                  </div>`;
                });
                $('.collection-chip-container').empty();
      ...
      collectionDynamic: function () {
        $('.collection-chip-container').on('click', '.chip', function (e) {
          let taxon = $(this).find('.taxon').text();
          let storedArray = localStorage.getItem('collections');
          storedArray = JSON.parse(storedArray);
          let items = storedArray.find(obj => obj.taxon === taxon);
          ...
          $('.collection-chip-container').fadeOut(500, function() {
            ...
            $('#displayName').val(items.displayName);
            $('#twitterAt').val(items.twitter);
            $('#descriptionInput').val(items.description);
            $('#logo-preview').prop('src', items.collectionLogo);
      ...
      submitCollection: async function () {
        $('#updateCollection').on('click', async function() {
...
function checkAllFieldsFilled() {
  const inputs = document.querySelectorAll('.details-inner.description input');
